...
\begin{document}
	...
	\cite{wangler}:
	\begin{align}
	\text{TM-Moden:} \quad E_z = E_0 J_m(k_{mn} r) \cos(m \theta) \cos\left(\frac{p \pi z}{l}\right) \exp(i \omega t) \qquad B_z = 0\\
	\text{TE-Moden:} \quad E_z = 0 \qquad B_z = B_0 J_m(k_{mn} r) \cos(m \theta) \sin\left(\frac{p \pi z}{l}\right) \exp(i \omega t)
	\end{align}
	\begin{align}
	k_{mn} = \frac{x_{mn}}{R}
	\end{align}
	mit Radius $R$ und Nullstelle $x_{mn}$ die $n$-te Nullstelle ($>0$) der Besselfunktion $J_m$
