...
Dies ist der Fall, da diese Moden ein verschwindendes elektrisches und magnetisches Feld in der mittleren Zelle des Resonators aufweisen (vgl.\ Abb.\ \ref{fig:spektrum_tm010}, \ref{fig:phasenbeziehung}) und daher nicht mit der Koppelschleife angeregt werden können.
...
Außerdem kann das effektive elektrische Feld, das ein ultrarelativistisches Teilchen erfährt, welches den Resonator passiert, berechnet werden und somit Laufzeitfaktor~$\Lambda$ \eqref{eq:laufzeitfaktor} bestimmen.
Dazu muss neben der harmonischen Zeitabhängigkeit auch die Phasenbeziehung (vgl.\ Abb.\ \ref{fig:phasor}) zwischen den einzelnen Zellen beachtet werden.
...
Zusätzlich sollen die longitudinalen Shuntimpedanzen~$R_\mathrm{S}$ dieser Moden bestimmt werden.
Diese folgen mit Gleichung \eqref{eq:beschleunigungsspannung} und \eqref{eq:shuntimpedanz} durch Integration der auf die Verlustleistung~$P_\mathrm{V}$ normierten Amplitude des elektrischen Feldes~$|\ve_0|$ über die Länge~$L$ des Resonators:
\begin{align}
	R_\mathrm{S} = \frac{1}{2} \left( \int_0^L \frac{|\ve_0(z)|}{\sqrt{P_\mathrm{V}}} \, \mathrm{d}z \right)^2
\end{align}
Die Integration über die experimentellen Daten erfolgt dabei numerisch durch die Trapezregel.
Um die effektive Shuntimpedanz der Moden zu bestimmen, muss außerdem der Laufzeitfaktor~$\Lambda$ berechnet werden.
Mit Gleichung \eqref{eq:laufzeitfaktor}, dem effektiven Feld und der Feldamplitude kann der Laufzeitfaktor berechnet werden.
Schließlich folgt die effektive Shuntimpedanz~$R_\mathrm{S}^\mathrm{eff}$ unter Verwendung von Gleichung \ref{eq:eff_shuntimpedanz}.
Diese Kenngrößen wurden in Tabelle \ref{tab:shuntimpedanzen_tm010} zusammengestellt.
\begin{table}[htb]
