...
Dazu dient ein Referenzpunkt in jeder Stempel-Zelle, an denen die Resonanzfrequenzverschiebung durch den Störkörper gemessen wird. 
Anschließend werden die Stempel antiparallel verfahren und die Resonanzfrequenzverschiebung an beiden Stellen erneut gemessen.
Eine symmetrische Feldverteilung ist erreicht, wenn an beiden Referenzpunkten die gleiche Frequenzverschiebung durch den Störkörper entsteht.

Schließlich muss die Beschleunigermode auf ihre Sollfrequenz im Vakuum von $\SI{499.67}{MHz}$ durch paralleles Verfahren der Stempel abgestimmt werden.
...
Dies ist der Fall, da diese Moden ein verschwindendes elektrisches und magnetisches Feld in der mittleren Zelle des Resonators aufweisen (vgl.\ Abb.\ \ref{fig:spektrum_tm010}, \ref{fig:phasenbeziehung}) und daher nicht oder nur schwach über die Koppelschleife angeregt werden können.
...
In Abbildung \ref{fig:cst_sim_tm111} wurden exemplarisch die (qualitativen) Ergebnisse einer Simulation der vermessenen $\mathrm{TM}_{111}$-Mode durch CST-MWS aufgetragen. 
\begin{figure}[h]
	\centering
	\begin{subfigure}{1\textwidth}
		\centering
		\includegraphics[width=1.0\textwidth]{./figs/TM111-CST/TM111_cut.png}
		\caption{Qualitative Darstellung des elektrischen Feldes zu einem festen Zeitpunkt. Gezeigt ist der Querschnitt des Resonator-Modells mit der Strahlenachse in der Ebene.}
	\end{subfigure}
	\begin{subfigure}{1\textwidth}
		\centering
		\hspace*{-8mm}\input{./plots/tm111_abs_sim.tex}
		\caption{Amplitude des elektrischen Feldes in Abhängigkeit der Position auf der Strahlenachse.}
	\end{subfigure}
	\caption{Simulation der vermessenen $\mathrm{TM}_{111}$-Eigenmode durch CST-MWS.}
...
Mit der Verwendung eines kleineren Störkörpers ist dennoch eine Möglichkeit gegeben diese Sprünge zu vermindern, was jedoch mit einer Abnahme der Frequenzverschiebung durch die Störung der Resonatormoden zusammenhängt.
...
Ferner ist die Auflösung mit der die Amplitude des elektrischen Feldes bestimmt werden kann durch den Zusammenhang
\begin{align}
\frac{|\ve_0|}{\sqrt{P_\mathrm{V}}} \propto \sqrt{\Delta \omega}
\end{align}
und der konstanten Frequenzauflösung des VNA beschränkt.
Dies führt zu einer Abnahme des Auflösungsvermögens bei kleinen Amplituden des elektrischen Feldes (oder kleinen Frequenzverschiebungen~$\Delta \omega$).
...
Der Grund dafür ist, dass in der mittleren Zelle ein elektrisches Feld existiert, welches unter die Auflösungsgrenze fällt.

Abschließend soll die Diskussion der effektiven Shuntimpedanzen der Resonatormoden erfolgen.
Die größte effektive Shuntimpedanz der im Rahmen dieser Arbeit vermessenen Moden zeigt die $\mathrm{TE_{111}}$-Mode mit $R_\mathrm{S}^\mathrm{eff} = \SI{326 +- 11}{\kilo\ohm}$ (transversal), was einem Geometriefaktor von \SI{29.21 +- 0.05}{\ohm} entspricht.

Die größte effektive Shuntimpedanz der $\mathrm{TM}_{021}$-Moden zeigt die Mode mit der Resonanzfrequenz $\nu_0 = \SI{1465.83}{MHz}$.
Diese liegt bei $R_\mathrm{S}^\mathrm{eff} = \SI{183.0 +- 6.4}{\kilo\ohm}$ und fällt damit zwei Größenordnungen unterhalb der effektiven Shuntimpedanz der Fundamentalmode.

Abschließend ist zu bemerken, dass keine der vermessenen Moden höherer Ordnung effektive Shuntimpedanzen in der Größenordnung der Fundamentalmode aufweist und daher die Gefahr kritische Multi-Bunch-Instabilitäten anzuregen, klein ist.
...
Bei einer weitergehenden Analyse ist eine Simulation des Impedanzspektrums (bsp CST Particle Studio) mit der korrekten Resonatorgeometrie zu empfehlen, da somit eine 
