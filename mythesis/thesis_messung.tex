...
Dazu muss neben der harmonischen Zeitabhängigkeit (analog zu Gl.\ \eqref{eq:laufzeitfaktor}) auch die Phasenbeziehung (vgl.\ Abb.\ \ref{fig:phasenbeziehung}) zwischen den einzelnen Zellen beachtet werden.
Darüber hinaus wird die Eintrittsphase des Teilchens in die Kavität so gewählt, dass dieses eine maximale Beschleunigungsspannung~$U_\mathrm{eff}$ erfährt.
...
Zusätzlich soll die longitudinale Shuntimpedanzen $R_\mathrm{S}$ der Moden bestimmt werden.
Diese folgt mit Gleichung \eqref{eq:beschleunigungsspannung} und \eqref{eq:shuntimpedanz} direkt durch Integration der auf die Verlustleistung~$P_\mathrm{V}$ normierte Amplitude des elektrischen Feldes~$|\ve_0|$
\begin{align}
	R_\mathrm{S} = \frac{1}{2} \left( \int_0^L \frac{|\ve_0(z)|}{\sqrt{P_\mathrm{V}}} \, \mathrm{d}z \right)^2
\end{align}
Laufzeitfaktor, effektive longitudinale Shuntimpedanz...
