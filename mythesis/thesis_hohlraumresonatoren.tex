...
Viele der in Beschleunigern verwendeten Kavitäten\footnote{Hohlraumresonator oftmals engl. \emph{Cavity}} basieren auf kreiszylindrischen Resonatoren\footnote{engl. \emph{Pillbox-Cavities}, für deren Ähnlichkeit mit einer Tablettenschachtel}.
...
Man findet für die Moden des kreiszylindrischen Resonators \cite{wangler}:
\begin{subequations}
  \begin{align}
...
\end{align}
Die Gleichungen \eqref{eqs:felder_pillbox} und \eqref{eq:frequenz_pillbox} gelten für den Idealfall geschlossener zylindrischer Resonatoren und beinhalten daher keine Eintritts-/Austrittsfenster für geladene Teilchen.
...
%------------------------------------------------------------------------------
\section{Kenngrößen von Resonatoren am Modell des RLC-Parallelschwingkreises}
\label{sec:rlc_schwingkreis}
%------------------------------------------------------------------------------
\begin{figure}[htb]
  \centering
...
\end{figure}
Die elektrischen Eigenschaften von Hohlraumresonatoren können in einem beschränkten Frequenzbereich um eine Resonanz durch das Modell des Parallelschwingkreises (siehe Abb.\ \ref{fig:rlc_circuit}) erklärt werden \cite{wangler, wille, pozar}.
Zu dessen vollständiger Beschreibung ist die Angabe der drei Kenngrößen \textsc{ohm}scher Widerstand~$R$, Induktivität~$L$ und Kapazität~$C$ ausreichend.
...
\begin{figure}[htb]
  ...
  \caption[Modell der induktiven Kopplung eines Hochfrequenzsignals über die Koppelschleife an den Resonator]{Modell der induktiven Kopplung eines Hochfrequenzsignals~$I_0$ über die Koppelschleife an den Resonator \cite{wangler}. Die angepasste Signalquelle ist mit einem Wellenleiter charakteristischer Impedanz~$Z_0$ mit der Koppelschleife (idealer Transformator) verbunden. Die Impedanz des Resonators~$Z(\omega)$ im Sekundärkreis wird durch die induktive Kopplung zur Impedanz~$Z^\prime(\omega)$ im Primärkreis transformiert.}
...
\end{align}
mit dem Wellenwiderstand~$Z_0$ des Wellenleiters und unterscheidet zwischen unterkritischer \mbox{$(\kappa < 1)$}, kritischer $(\kappa = 1)$ und überkritischer Kopplung $(\kappa > 1)$.
...
Mit der Definition des Koppelfaktors~$\kappa$, der Proportionalität $Z^\prime(\omega) \propto Z(\omega)$ für ideale Transformatoren und der Impedanz des Resonators im Resonanzfall $Z(\omega_0) = R$ folgt die Frequenzabhängigkeit der Impedanz~$Z^\prime(\omega)$ des Systems bestehend aus Koppelschleife und Resonator \cite{hillert}:
\begin{align}
  Z^\prime(\omega) = \frac{Z^\prime(\omega_0)}{Z(\omega_0)} \cdot Z(\omega) = \kappa \frac{Z_0}{R} Z(\omega) \eqdot
