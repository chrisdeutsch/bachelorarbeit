...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.3\textwidth]{./figs/RLC_circuit.pdf}
  \caption{RLC Parallelschwingkreis \todo{Bild mit Koppelschleife}}
...
\end{align}
mit dem Wellenwiderstand~$Z_0$ des Wellenleiters und unterscheidet zwischen unterkritischer $(\kappa < 1)$, kritischer $(\kappa = 1)$ und überkritischer Kopplung $(\kappa > 1)$ \todo{Wichtig: Bei Resonanzfrequenz}.
...
Mithilfe des Koppelfaktors kann die Impedanz~$Z^\prime$ in Abhängigkeit der Kreisfrequenz~$\omega$ angegeben werden\footnote{Für ideale Transformatoren ist die Impedanz am Sekundärkreis $Z^\prime(\omega)$ proportional zur Impedanz im Primärkreis $Z(\omega)$}:
\begin{align}
